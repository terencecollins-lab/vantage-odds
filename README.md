...
- `server.py` — stdlib-only Python server. Serves the static frontend and exposes:
  - `/api/markets?league=NFL` — fetches + normalizes upcoming odds into card view-models (best price, fair odds, edge %, no-vig probability, per-bookmaker rows), dropping any game whose own start time is more than 60 minutes in the past (`_game_started_too_long_ago`) — once a game's well underway its pre-game props are stale and not worth continuing to serve or fetch mini-form data for. This filter lives here rather than in a separate step because MLB Matchups and KBO Matchups build their own "pick a game" dropdowns from this same endpoint's Moneyline items, so filtering here also stops those tabs from offering an in-progress game that would trigger a fresh full-roster scrape. Cached 20s.
